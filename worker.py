...
from reward import getGameState, formatMoves
...
class Worker():
    def __init__(self, port, number, agent):
        ...
        def worker():
            subprocess.call(['./runGameDebugConfig.sh', str(self.port)]) # runSimulation
    ...
    def work(self, sess, coord, saver, n_simultations):
        ...
        with sess.as_default(), sess.graph.as_default():
            for i in range(n_simultations): # while not coord.should_stop():
                ...
                if self.number == 0:
                    saver.save(sess, 'models/'+self.agent.name)
                    self.agent.experience.save_metric('models/'+self.agent.name)
